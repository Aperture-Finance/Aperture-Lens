...
import {IPCSV3NonfungiblePositionManager as IPCSV3NPM, INonfungiblePositionManager as INPM} from "@aperture_finance/uni-v3-lib/src/interfaces/INonfungiblePositionManager.sol";
...
/// @notice Base contract for PancakeSwap v3 that peeks into the current state of position and pool info
abstract contract PCSV3PositionUtils is PoolUtils {
    ...
    function peek(INPM npm, uint256 tokenId, PCSV3PositionState memory state) internal view {
        ...
        V3PoolCallee pool = V3PoolCallee.wrap(
            PoolAddressPancakeSwapV3.computeAddressSorted(
                IPCSV3NPM(address(npm)).deployer(),
                position.token0,
                position.token1,
                position.fee
            )
    ...
    function slot0InPlace(V3PoolCallee pool, PCSV3Slot0 memory s) internal view {
