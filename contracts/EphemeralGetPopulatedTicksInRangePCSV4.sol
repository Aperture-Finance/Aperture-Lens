...
import "./PoolUtilsPCSV4.sol";

/// @notice A lens that fetches chunks of tick data in a range for a PancakeSwap Infinity pool without deployment
...
contract EphemeralGetPopulatedTicksInRangePCSV4 is PoolUtilsPCSV4 {
        ...
        returns (PopulatedTick[] memory populatedTicks)
    {
        ...
        unchecked {
            (uint256[] memory tickBitmap, uint256 count) =
                getTickBitmapAndCount(ICLPoolManager(poolManagerAddr), poolKey, wordPosLower, wordPosUpper);
    ...
    function populateTick(
        ...
        Tick.Info memory tickInfo = ICLPoolManager(poolManagerAddr).getPoolTickInfo(poolKey.toId(), tick);
        populatedTick.tick = tick;
        populatedTick.liquidityGross = tickInfo.liquidityGross;
        populatedTick.liquidityNet = tickInfo.liquidityNet;
        populatedTick.feeGrowthOutside0X128 = tickInfo.feeGrowthOutside0X128;
        populatedTick.feeGrowthOutside1X128 = tickInfo.feeGrowthOutside1X128;
    }
}
